...
class TestPatternMergeIntervals < Test::Unit::TestCase
  def test_merge_interval
    assert_equal([[1, 5], [7, 9]], merge_intervals(array_test_case_one))
    assert_equal([[2, 4], [5, 9]], merge_intervals(array_test_case_two))
    assert_equal([[1, 6]], merge_intervals(array_test_case_three))
  end
end
